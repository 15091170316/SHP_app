    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <ZoomVue :skuImageList="skuInfo.skuImageList || [{}]" />
          <!-- 小图列表 -->
            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr of spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: spuSaleAttrValue.isChecked == 1 }"
                  @click="chooseHandler(spuSaleAttrValue, spuSaleAttr.spuSaleAttrValueList)"
                  v-for="spuSaleAttrValue of spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id">
                  {{ spuSaleAttrValue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @blur="blurHandler">
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum > 1 ? skuNum-- : skuNum = 1">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="addShoppingCarHandler">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions('detail', ['getGoodInfo', 'addUpdateShoppingCar']),
    // 商品参数选择点击事件
    chooseHandler(spuSaleAttrValue, arr) {
      arr.forEach(item => {
        item.isChecked = 0
      });
      spuSaleAttrValue.isChecked = 1
    },
    // 购买的商品数量失去焦点后要进行非法输入校验
    blurHandler() {
      let num = this.skuNum * 1 //转换为数字类型
      if (isNaN(num) || num < 1) {
        this.skuNum = 1
      }
      if (num >= 1) {
    ...
    // 添加购物车事件
    async addShoppingCarHandler() {
      // 生成随机游客的token(存在localstorage中)
      if(!localStorage.getItem('nanoid_token')){
        localStorage.setItem('nanoid_token', nanoid())
      }
      // 成功
      try {
        await this.addUpdateShoppingCar({ skuId: this.$route.params.goodId, skuNum: this.skuNum })
        // 跳转到添加购物车页面,传递商品购买个数
        this.$router.push({path: '/addcart', query:{skuNum: this.skuNum}})
        // 将购买的商品信息存在会话存储中（因为是对象类型数据，所以最好不要使用路由传参的形式发送）
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo)) //对象要转成JSON格式数据
      } catch (error) {   //失败
        alert(error.message);
      }
    }
  ...
  mounted() {
    this.getGoodInfo(this.$route.params.goodId)
  }
}
</script>
