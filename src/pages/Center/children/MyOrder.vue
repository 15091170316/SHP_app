<template>
    <div class="order-right">
        <div class="order-content">
            ...
            <div class="orders">
                <table class="order-item" v-for="item of this.records" :key="item.id">
                    <thead>
                        <tr>
                            <th colspan="5">
                    ...
                    <tbody>
                        <tr v-for="orderDetail, index of item.orderDetailList" :key="orderDetail.id">
                            <td width="60%">
            ...
            <!-- 分页系统 -->
            <PaginationVue v-if="myOrderInfo.total!==0" @curPageNo="curPageNo" :pageNo="pageInfo.page" :pageSize="pageInfo.limit" :total="myOrderInfo.total" :continues="5"></PaginationVue>
        </div>
        <!--猜你喜欢-->
        <div class="like">
            <h4 class="kt">猜你喜欢</h4>
...
<script>
import { reqMyOrderList } from '@/api/api'
export default {
    name: 'MyOrder',
    data() {
        return {
    ...
    methods: {
        // 获取我的订单数据
        async getOrderInfo() {
            let result = await reqMyOrderList(this.pageInfo)
            console.log(result);
            if (result.code === 200) {
                this.myOrderInfo = result.data
                this.pageInfo.page = result.data.current
            }
        ...
        curPageNo(page){
            ...
            this.getOrderInfo() //请求数据
        }
    },
    mounted() {
        // 获取我的订单数据，封装到其他函数中（生命周期钩子中尽量不要有async await）
        this.getOrderInfo()
    }
}
</script>
